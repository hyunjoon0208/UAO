...
import torch
import torchvision
from torchvision import datasets, transforms  # 이 import 추가
...
#             root='/workspace/data/ImageNet/train',
#             transform=train_transform
#         )
#         validation_set = datasets.ImageFolder(
#             root='/workspace/data/ImageNet/val',
...
def prepare_data(
    ...
    if dataset == "imagenet":
        ...
        if train_subset_indices is None and num_indexes_to_replace is not None:
            total_samples = len(train_set)
            rng = np.random.RandomState(seed)
            selected_indices = rng.choice(total_samples, num_indexes_to_replace, replace=False)
            
            train_subset_indices = torch.ones(total_samples, dtype=torch.long)
            train_subset_indices[selected_indices] = 0
    ...
    if train_subset_indices is not None:
        loaders = {
            ...
            "fog": DataLoader(
                ...
                shuffle=(forget_sampler is None and shuffle)
            ),
        }
    else:
        loaders = {
...
if __name__ == "__main__":
    ...
    loaders = prepare_data(
        dataset="imagenet", batch_size=1, shuffle=False, data_path="/workspace/data/"
    )
