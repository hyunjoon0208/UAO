...
def save_checkpoint(
    state, is_SA_best, save_path, filename="checkpoint.pth.tar"
):
    filepath = os.path.join(save_path, filename)
    torch.save(state, filepath)
    if is_SA_best:
        shutil.copyfile(
            filepath, os.path.join(save_path, "model_SA_best.pth.tar")
        )
...
def setup_model_dataset(args, rank=-1, world_size=1):
    if args.dataset == "imagenet":
        ...
        if args.unlearn == "retrain":
            model = model_dict[args.arch](num_classes=classes, pretrained=False)
        else:
            model = model_dict[args.arch](num_classes=classes, pretrained=True)
        model.normalize = normalization
        
        if args.debug:
            print("!!! DEBUG MODE ENABLED: Reducing num_indexes_to_replace to 1000 !!!")
            args.num_indexes_to_replace = 1000
            
        train_ys = torch.load(args.train_y_file)
        val_ys = torch.load(args.val_y_file)
        if args.class_to_replace != -1:
            forget_indices = (train_ys == args.class_to_replace).nonzero(as_tuple=True)[0]
            
            train_subset_indices = torch.ones_like(train_ys)
            train_subset_indices[forget_indices] = 0
            ...
            train_full_loader = loaders["train_full"]
            retain_loader = loaders["train"]
            forget_loader = loaders["fog"]
            val_loader = loaders["val"]
            return model, train_full_loader, retain_loader, val_loader, forget_loader
        ...
        if args.num_indexes_to_replace is None:
            ...
            forget_indices = torch.randperm(total_samples)[:args.num_indexes_to_replace]
            
            train_subset_indices = torch.ones_like(train_ys)
            val_subset_indices = torch.ones_like(val_ys)
            
            train_subset_indices[forget_indices] = -1
