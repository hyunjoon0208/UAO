...
def save_detailed_results(evaluation_result, args, save_path):
    current_time = datetime.now().strftime("%Y%m%d_%H%M%S")
    
    experiment_config = {
        "dataset": args.dataset,
        "model": args.arch,
        "num_indexes_to_replace": getattr(args, "num_indexes_to_replace", None),
        "class_to_replace": getattr(args, "class_to_replace", None)
    }
    ...
    filename = f"{args.dataset}_{args.arch}_{args.unlearn}_{current_time}_results.txt"
    full_save_path = os.path.join(save_path, filename)
    
    with open(full_save_path, "w", encoding="utf-8") as f:
        f.write(f"실험 시간: {current_time}\n\n")
...
def save_unlearn_checkpoint(model, evaluation_result, args):
    ...
    save_detailed_results(evaluation_result, args, args.save_dir)
